...
moduleForComponent('post-edit-form', 'Integration | Component | project post new form', {
  integration: true
});
...
test('it renders', function(assert) {
  ...
  assert.equal(this.$('.post-edit-form').length, 1, 'The component\'s element renders');
});
...
test('it triggers an action when the post is saved', function(assert) {
  ...
  this.render(hbs`{{post-edit-form post=post savePost='savePost'}}`);
